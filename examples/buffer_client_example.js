...
.then(() => {
  ...
  const bufferClient = new tf2Nodejs.BufferClient('tf_action');
  ...
  function query() {
    rl.question('Transform between which frames?\n', (answer) => {
      ...
      if (frames.length === 2) {
        ...
        // FIXME: bufferClient lookupTransform is supposed to take a transform object, not an id
        bufferClient.lookupTransform(frames[0], frames[1], now)
        .then((result) => {
